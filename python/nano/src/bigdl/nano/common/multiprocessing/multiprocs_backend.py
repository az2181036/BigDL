...
class MultiprocessingBackend(Backend):
    ...
    def run_process(self, target, args=..., nprocs=1, envs=None) -> None:
        ...
        for i in range(nprocs):
            ...
            for key, value in envs[i].items():
                if key in os.environ and key not in env_back:
                    env_back[key] = os.environ[key]
                else:
                    env_del_list.append(key)
                os.environ[key] = value

            p = Process(target=target, args=[args]+[envs[i]])
            p.start()
            proc_list.append(p)

        for idx, p in enumerate(proc_list):
            p.join()
            print(f"process {idx} exitcode: {p.exitcode}")
    ...
    def run_pool(self, target, args=..., nprocs=1, envs=None) -> None:
        ...
        result = []
        for i in range(nprocs):
            res = self._pool.apply_async(func=target, args=[args]+[envs[i]])
            result.append(res)
